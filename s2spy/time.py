...
class AdventCalendar:
    ...
    def _mark_target_period(
        ...
        Example:
            ...
            >>> import numpy as np
            >>> import pandas as pd
            >>> calendar = s2spy.time.AdventCalendar(anchor_date=(5, 10), freq='100D', n_targets = 2)
            >>> df = calendar.map_years(2018, 2020)
            >>> df = df.melt(var_name="i_interval", value_name="interval", ignore_index=False)
            >>> df = df.sort_values(by=["anchor_year", "i_interval"])
