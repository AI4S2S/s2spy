...
class AdventCalendar(BaseCalendar):
    ...
    def get_intervals(self) -> pd.DataFrame:
        ...
        self._targets = [TargetPeriod(self.freq) for i in range(self.n_targets)]
        periods = self._get_nintervals()
        self._precursors = [
            PrecursorPeriod(self.freq) for i in range(periods - self.n_targets)
        ]

        # calculate the length
        self._total_length_target = pd.Timedelta(
            self.n_targets * int(self.freq.replace("d", "")), unit="D"
        )
        self._total_length_precursor = pd.Timedelta(
            (periods - self.n_targets) * int(self.freq.replace("d", "")), unit="D"
        )
